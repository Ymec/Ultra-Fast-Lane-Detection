...
if __name__ == "__main__":
    ...
    # cap = cv2.VideoCapture("./data/2020-10-10-15-00-31.mp4")
    cap = cv2.VideoCapture("./data/2020-10-22-14-15-18-culane.mp4")
    frame = cap.read()
    fourcc = cv2.VideoWriter_fourcc(*'MJPG')
    videoname = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    # vout = cv2.VideoWriter('output/' + videoname + '.avi', fourcc, 30.0, (1280, 720))
    vout = cv2.VideoWriter('output/'+videoname + '_culane_50.avi', fourcc, 20.0, (1640, 590))
    from PIL import Image

    print('load cuda：', torch.cuda.is_available())
    while 1:
        rval, frame = cap.read()
        if rval == False:
            break
        ...
        for i in range(out_j.shape[1]):
            if np.sum(out_j[:, i] != 0) > 2:
                for k in range(out_j.shape[0]):
                    if out_j[k, i] > 0:
                        ppp = (int(out_j[k, i] * col_sample_w * 1640 / 800) - 1,
                               int(590 * (culane_row_anchor[cls_num_per_lane - 1 - k] / 288)) - 1)
